...
{$imgIns = false}
{$fileIns = false}
{if $files && php_isset($files.image) && php_count($files.image) > 0}{$imgIns = true}{/if}
{if $files && php_isset($files.other) && php_count($files.other) > 0}{$fileIns = true}{/if}
	...
	<div class="attachmentManage imageManage">
		...
		{foreach $files.image as $file}
			<li id="item-file-{$file->id}" class="image item">
				<img src="{thumb($file, 'scaley', '32')}" alt="" />
