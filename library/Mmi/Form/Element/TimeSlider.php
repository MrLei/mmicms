		...
		$view->headScript()->appendScript('
			$(document).ready(function() {
				$(\'#' . $this->id . 'Slider\').slider({range: "min", \'value\': ' . $this->value . ', \'min\': ' . $min . ',\'max\': ' . $max . ', '.(($step)? '\'step\' : '.$step.' ,' : '').' slide: function(event, ui) {
						...
						$(\'#' . $this->id . '_container div.ui-slider-range\').html(\'<div class="float-label">\'+ui.value+\' h</div>\');
					}
				});
			});
		');
		$html = '<input class="sliderField" ';
		$html .= 'type="hidden" ' . $this->_getHtmlOptions() . '/><p class="slider"><span class="slider" id="' . $this->id . 'Slider"></span><span class="sliderTo max">' . number_format($max, 0, ',', ' ') . ' h</span></p>';
		return $html;
	}
