$(document).ready(function () {
	...
	function bindSortable() {
		...
		$("#manageImage").sortable({
			...
			update: function(event, ui) {
				$.post(
					request.baseUrl + "/?module=cms&controller=admin-file&action=sort",  $(this).sortable('serialize'),
					function(result) {
						if (result) {
							alert(result);
						}
	...
	$('body').on('click', 'a.edit-file', function () {
		...
		$.getJSON(
			request.baseUrl + '/?module=cms&controller=admin-file&action=edit&id=' + id[2] + '&hash=' + id[3],
			function(result) {
				...
				$('#fileUpload').hide();
				$('#uploaderEdit').show();
				$('#uploaderEditForm').attr('action', request.baseUrl + '/?module=cms&controller=admin-file&action=edit&id=' + id[2] + '&hash=' + id[3]);
	...
	$('body').on('click', 'a.remove-file', function () {
		...
		var id = $(this).attr('id').split('-');
		$.get(
			request.baseUrl + '/?module=cms&controller=admin-file&action=delete&id=' + id[1] + '&hash=' + id[2],
			function(result) {
				if (!result) {
					location.reload(); 
	...
	$('body').on('click', 'input.sticky', function () {
		var id = $(this).attr('id').split('-');
		$.get(
			request.baseUrl + '/?module=cms&controller=admin-file&action=stick&id=' + id[2] + '&hash=' + id[3],
			function(result) {
				if (result) {
					alert(result);
				}
