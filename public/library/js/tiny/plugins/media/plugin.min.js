...
tinymce.PluginManager.add('media', function(editor, url) {
	...
	function dataToHtml(data) {
		...
		if (data.embed) {
			...
			if (data.type == "iframe") {
				var ap, vid = data.orginal.split('v=');
				if(vid.length !== 0) {
					vid=vid[1];
					ap=vid.indexOf('&');
					if(ap != -1) {
						vid = vid.substring(0, ap);
					}
					data.source1 = 'http://img.youtube.com/vi/' + vid + '/hqdefault.jpg'
				}
				html += '<img src="' + data.source1 + '" width="' + data.width + '" height="' + data.height + '" data-type="youtube" data-source="'+vid+'">';
